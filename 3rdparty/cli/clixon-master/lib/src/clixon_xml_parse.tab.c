...
{
       0,   321,   321,   324,   329,   332,   334,   336,   339,   340,
     343,   347,   349,   353,   355,   356,   355,   361,   364,   369,
     370,   373,   374,   375,   377,   380,   384,   385,   388,   391,
     392,   397,   398
};
#endif
...
{
  "$end", "error", "$undefined", "NAME", "CHARDATA", "VER", "ENC",
  "BSLASH", "ESLASH", "BTEXT", "ETEXT", "BCOMMENT", "ECOMMENT", "'='",
  "'\"'", "'\\''", "'<'", "':'", "'>'", "$accept", "topxml", "dcl", "info",
  "encode", "element", "qname", "element1", "$@1", "$@2", "etg", "list",
  "content", "comment", "attrs", "attr", "attqname", "attvalue", YY_NULLPTR
};
#endif
...
static const yytype_uint8 yydefact[] =
{
      ...
      22,     0,     0,    25,    11,    27,     1,     3,    19,     0,
       0,     0,     0,     0,     0,     0,     0,     4,    12,    29,
      13,    14,    10,    26,     0,     0,     0,     0,     0,     0,
      24,     0,     5,     6,     0,     0,    30,    15,     0,    28,
...
yyreduce:
  ...
  case 4:
    ...
    { clicon_debug(3, "dcl->info encode"); }

    break;

  case 5:

    { if (xml_parse_version(_YA, (yyvsp[-1].string)) <0) YYABORT; }
  ...
  case 11:
    ...
    { if (xml_parse_qname(_YA, NULL, (yyvsp[0].string)) < 0) YYABORT; 
                                clicon_debug(3, "qname -> NAME %s", (yyvsp[0].string));}

    break;

  case 12:

    { if (xml_parse_qname(_YA, (yyvsp[-2].string), (yyvsp[0].string)) < 0) YYABORT; 
  ...
  case 15:
    ...
    { xml_parse_endslash_mid(_YA); }

    break;

  case 16:

    { xml_parse_endslash_post(_YA); 
  ...
  case 17:
    ...
    { 			   clicon_debug(3, "etg -> < </ NAME %s>", (yyvsp[-1].string)); if (xml_parse_bslash1(_YA, (yyvsp[-1].string)) < 0) YYABORT; }

    break;

  case 18:

    { if (xml_parse_bslash2(_YA, (yyvsp[-3].string), (yyvsp[-1].string)) < 0) YYABORT; 
  ...
  case 19:
    ...
    { clicon_debug(3, "list -> list content"); }

    break;

  case 20:

    { clicon_debug(3, "list -> content"); }

    break;

  case 21:

    { clicon_debug(3, "content -> element"); }

    break;

  case 22:

    { clicon_debug(3, "content -> comment"); }

    break;

  case 23:

    { if (xml_parse_content(_YA, (yyvsp[0].string)) < 0) YYABORT;  
                             clicon_debug(3, "content -> CHARDATA %s", (yyvsp[0].string)); }
  ...
  case 28:
    ...
    { if (xml_parse_attr(_YA, (yyvsp[-2].string), (yyvsp[0].string)) < 0) YYABORT; }
  ...
  case 30:
    ...
    { if (((yyval.string) = xml_merge_attqname(_YA, (yyvsp[-2].string), (yyvsp[0].string))) == NULL) YYABORT; }

    break;

  case 31:

    { (yyval.string)=(yyvsp[-1].string); /* $2 must be consumed */}

    break;

  case 32:

    { (yyval.string)=strdup(""); /* $2 must be consumed */}
